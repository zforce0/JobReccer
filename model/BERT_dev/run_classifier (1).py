...
from edited_roberta import RobertaForSequenceClassification
...
def load_and_cache_examples(args, task, tokenizer, ttype='train'):
    ...
    print('cached_features_file', cached_features_file, os.path.exists(cached_features_file))
    ...
    all_query_token_ids = torch.tensor([f.query_token_ids for f in features], dtype=torch.long)
    all_code_token_ids = torch.tensor([f.code_token_ids for f in features], dtype=torch.long)
    all_input_mask = None#torch.tensor([f.input_mask for f in features], dtype=torch.long)
    all_segment_ids = None#torch.tensor([f.segment_ids for f in features], dtype=torch.long)
    #if output_mode == "classification":
    all_label_ids = torch.tensor([f.label_id for f in features], dtype=torch.float)
        

    dataset = TensorDataset(all_query_token_ids, all_code_token_ids, all_query_token_ids, all_label_ids)
    if (ttype == 'test'):
...
def main():
    ...
    parser.add_argument("--task_name", default='jobsearch', type=str, required=True,
